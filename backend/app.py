...
app = Flask(__name__)
CORS(app, resources={r"/*": {"origins": "*"}})  # Allow all origins (Vercel <-> Render)
...
def extract():
    ...
    prompt = (
        "Extract the key clauses from the following contract. Return a JSON object with keys: "
...
def risks():
    ...
    # Fallback: heuristic parsing if not valid JSON
...
def compare():
    ...
    prompt = f"Compare these two contracts and list the key differences in plain English.\n\nContract A:\n{text1[:2000]}\n\nContract B:\n{text2[:2000]}"
...
def qa():
    ...
    prompt = f"Answer the question based on the following contract.\n\nQuestion: {question}\n\nContract:\n{text[:3000]}"
